...
- When building login and registering functionality I realized I have calls to the Datetime class for datetime.now and .utcnow. This makes it untestable unless I implement a knows fix.

### Code decisions
- In implementing the create functionality of CRUD I realized that writing a simple create function from API to database was not going to be what I wanted due to what I expect would be a massive influx of Create queries down the line. So instead I wrote the endpoint and the logic between the API and the database to be able to accept multiple create calls at the same time.
...
- Added tags to the queries that go to the database via Entity Framework Core. More to see if I know what to write for tags (I don't really, I imagine a database architect has a better handle on that) or if simply writing the location of the code in the database that executes the query is better (for now I did not do this). 
- Implemented a Datetimewrapper class to deal with the problem of untestable code due to calls to Datetime.now and .utcnow.
